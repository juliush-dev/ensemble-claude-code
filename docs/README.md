...
| `settings.json` | Default model, memory settings, hook wiring (including the inline compaction marker and the harness marker), and curated tool-permission rules. The deploy merges this file into the home's existing one, so keys the home added itself survive an update. |
