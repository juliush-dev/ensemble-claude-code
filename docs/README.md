...
| `skills/` | Protocol runbooks and curated craft skills, each invoked at its moment. Every agent card preloads `decision-proposal-discipline` and `writing-and-talking-style`. |
| `hooks/` | Small shell guards (Git Bash) — a session-end litter flag, the Scout's one-command guard, the main session's path guard, a push gate, the live-reads guard, which lets the Operator's browser, Gmail and Drive reads run without a prompt (the auto-mode classifier still judges them) and asks on every other call to those tools, a harness marker that adds one line to the session when the installed Claude Code is older than the version `HARNESS.md` names, and a session cue. When a session starts, resumes, is cleared or forks, the cue walks up from its folder: inside a wrapped project it tells the session to load the `onboard` runbook before working, where it finds no project face it tells the session to ask the human before working, and every time it adds a line to load `decision-proposal-discipline` before the first ask. It recognizes the shared external brain, where no face is expected, by a note at the vault's root whose name begins `Shared Long-Term Memory` and ends `Purpose.md`; an adopter's brain without such a note gets the ask-first line. |
...
| `HARNESS.md` | The Claude Code version this COS was last checked against, on its first line, and the harness fixes each guard relies on. The deploy copies it to the config home, where the harness marker reads it. |
| `settings.json` | Default model, memory settings, hook wiring (including the inline compaction marker, the harness marker and the session cue), and curated tool-permission rules. The deploy merges this file into the home's existing one, so keys and list entries the home added itself survive an update. |
