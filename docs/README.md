...
| `settings.json` | Default model, memory settings, hook wiring (including the inline compaction marker), and curated tool-permission rules. The deploy merges this file into the home's existing one, so keys the home added itself survive an update. |
| `merge-settings.py` | The one settings merge both deploy scripts call. It replaces the paths the factory declares, prunes the paths it has retired, and leaves every other key on the host alone. Requires Python 3. |
...
| `settings.local.example.json` · `settings.local.example.md` | The tracked example of the host companion `settings.local.json`, which carries machine-specific settings such as `permissions.additionalDirectories`, and the guidance that goes with it. |
| `skills-shipped.txt` · `fixtures/settings-merge/` | The append-only list of every skill this COS has ever shipped, which gates the skill prune the way `settings-shipped.txt` gates the settings prune; and the fixture that tests `merge-settings.py` (`python3 run-fixture.py`, seven cases, nothing written outside a temp directory). Neither lands in the config home: the deploy reads the manifest from the clone, and the fixture is run there by hand. |
...
- **Claude Code**, installed and on the `PATH`. The launcher calls `claude`.
...
`settings.json` is the one file the deploy merges rather than copies. `merge-settings.py` takes the config home's existing file as the base and changes exactly two things in it. The paths the factory declares now are replaced with the factory's values. The paths `settings-shipped.txt` records the factory as having declared in an earlier release and dropped since are removed. Nothing else is touched: every other key survives the update, wherever it sits in the tree. A plugin toggle, a notification preference, a model entry the harness wrote itself, an allow rule added through `/permissions` (a path the factory declares nowhere): all of them stay, where before this they were flattened on every update. The factory owns the paths it declares, so a hand-added entry inside one of those (a `deny` rule, say) does not survive, and the deploy names every such entry before it writes rather than letting it be discovered afterwards. The durable place for host-specific values is the gitignored companion `settings.local.json`, whose declared paths are re-applied on every update; `settings.local.example.md` explains it. After writing, the deploy re-reads the deployed file from disk and asserts path by path that it carries what the factory declares. The merge reads the config home's file and writes it back, with nothing locking it between the two, so a session that writes `settings.json` in that window loses that write. Deploy while no session is running.
